...
gg_supplyplan_overall <- function(countrygroup){
supplyplan |>
  filter(
  ...
  scale_fill_manual(values = c(
    ...
    "#f6e8c3", # Govt
    "#c7eae5", # UNFPA
    "#5ab4ac", # UNFPA CO financing
    "#01665e"  # USAID 
  )) + 
  labs(
    caption = str_c("Data source: Supply plans from the GFPVAN\nData downloaded:",
...
gg_supplyplan_country <- function(countrygroup){
  supplyplan |>
  filter(
    prh_countrygroup == countrygroup,
    collab_review_status %in% c(
...
#    title = "",
#    subtitle = "",
    caption = str_c("Data Source: Supply plans from the Global Family Planning Visibility & Analytics Network (GFPVAN)\nData downloaded on:\nData represents requirements for the next 4 quarters",
      importdate_pds,
      sep = " "
    ),
    x = "",
...
gg_supplyplan_overall("High Need") 
```
...
gg_supplyplan_country("High Need") +
...
gg_supplyplan_overall("Strategic Support")
```
...
gg_supplyplan_country("Strategic Support")
```
...
gg_supplyplan_overall("Strategic Transition")
...
gg_supplyplan_country("Strategic Transition")
```
